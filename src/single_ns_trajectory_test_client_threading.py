...
def create_arm_goals():
    ...
    for dict in trajectories['results']['path']: # This iterates 18 times, which makes sense...
        ...
        for sub_traj in dict['trajectory']: # This iterates six times, 
            ...
            time_final = sub_traj['0']['time_step']
...
def execute():
    base_goals = create_base_goals()
    arm_goals = create_arm_goals()

    # print("size of base goals: ", len(base_goals))
    # print("size of arm goals", len(arm_goals))

    client_base_2 = move_base_client('/TBwOM_2/movebase')
    client_arm_2 = move_arm_client('/TBwOM_2/movearm')

    for i in range(0, len(base_goals[:50])):
        goal_base_2 = base_goals[i]
        goal_arm_2 = arm_goals[i]

        print("goal base 2: ", goal_base_2 , " goal arm 2: ", goal_arm_2)
